...
import com.joka.zzzfun.http.listener.DefaultFileDownloanListener;
import okhttp3.*;
...
public final class OkClient implements HttpInterface {
    ...
    public OkClient(final Dns dns, final boolean hostFirst, final ProxyConfiguration proxy,
    ...
    public ZResponse multipartPost(String url, StringMap fields, String name, String fileName, byte[] fileBody, String mimeType, StringMap headers) {
    ...
    public ZResponse multipartPost(String url, StringMap fields, String name, String fileName, File fileBody, String mimeType, StringMap headers) {
        RequestBody file = RequestBody.create(MediaType.parse(mimeType), fileBody);
        return multipartPost(url, fields, name, fileName, file, headers);
    }
    ...
    public ZResponse send(final Request.Builder requestBuilder, StringMap headers) throws RuntimeException {
        ...
        completeHeader(requestBuilder,headers);
    ...
    public void donwloanFile(final Request.Builder requestBuilder, StringMap headers){
        ...
        IpTag tag = new IpTag();
        try {
            client.newCall(requestBuilder.tag(tag).build()).enqueue(new FileDownloadCallBack(new DefaultFileDownloanListener(),"tmp"));
        }catch (Exception e){
            e.printStackTrace();
        }
    ...
    private void completeHeader(Request.Builder requestBuilder, StringMap headers){
        if (headers != null) {
